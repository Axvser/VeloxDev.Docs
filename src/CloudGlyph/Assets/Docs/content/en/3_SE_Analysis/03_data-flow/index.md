...
| [04 Dynamic Theme](04_dynamic-theme) | `InitializeTheme` registration + converter pipeline → `PrepareSamplers` groups → one `CreateScheduler` per target on a shared `TransitionTimeline` (animated), or `ApplyImmediately` (`Jump`) → runtime overrides (`SetThemeValue`/`RestoreThemeValue`) |
