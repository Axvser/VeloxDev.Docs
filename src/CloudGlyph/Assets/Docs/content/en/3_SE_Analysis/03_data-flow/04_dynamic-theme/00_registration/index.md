...
The value per theme is computed once, at the first `InitializeTheme` of the type (inside the `IsTypeRegistered == false` branch), and stored as a `Dictionary<Type, object?>` per property. The `ThemeCache.RegisterConverter`/`GetConverter` registry exists for reuse, but the current generator instantiates converters inline (`Activator.CreateInstance`) instead of consulting it — `Src/Generators/VeloxDev.Core.Generator/Theme.cs`, lines 232-239.

Arity coverage: the generator discovers decorated classes through `ForAttributeWithMetadataName` metadata names for generic arities 3–7 (a converter plus two to six theme markers; `Theme.cs`, lines 32-55). The attribute family additionally defines the seven-theme variant (generic arity 8); nothing scans for that arity, but the emission reads **every** attribute on a discovered class whose name starts with `ThemeConfigAttribute` (`Theme.cs`, lines 90-92) and takes its theme types from the generic arguments from index 1 onward (`Theme.cs`, lines 160-224), so an arity-8 declaration on a class that also carries a discovered arity is registered like any other. The demos and tests exercise the two-theme arity-3 form (`ThemeConfigAttribute<BrushConverter, Light, Dark>`).

Both demos register themselves the same way — a `[ThemeConfig]` partial plus one `InitializeTheme()` call after `InitializeComponent()` — but they place the two global settings differently. The trimmed demo keeps them next to the call (`Examples/Theme/WPF Trimmed/Demo/MainWindow.xaml.cs`, `LoadTheme`); the scale demo has no window-level setup at all and puts `SetPlatformInterpolator`, `StartModel` and `SetCurrent<Dark>` in `App.OnStartup`, before any element registers itself (`Examples/Theme/WPF/Demo/App.xaml.cs`). The scale demo's tiles register from their own constructor (`Examples/Theme/WPF/Demo/ThemeTile.cs`, `ThemeTile()`), so building $N$ of them is the whole of "put $N$ targets into the next switch".

> Source: `Src/Generators/VeloxDev.Core.Generator/Theme.cs` (arity providers lines 32-55; attribute filtering lines 90-92; emitted `InitializeTheme` lines 387-415), `Src/Core/VeloxDev.Core/DynamicTheme/ThemeCache.cs` (`IsTypeRegistered` lines 41-47, `RegisterType` lines 54-68, `GetStaticForType` lines 98-103), `Src/Core/VeloxDev.Core/DynamicTheme/ThemeManager.cs` (`Register` lines 74-82), `Examples/Theme/WPF Trimmed/Demo/MainWindow.xaml.cs` (attribute lines 36-37, `LoadTheme` lines 40-53), `Examples/Theme/WPF/Demo/App.xaml.cs`, `Examples/Theme/WPF/Demo/ThemeTile.cs`.
