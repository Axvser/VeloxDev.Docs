...
**Expected result:** the `.csproj` gains a `<PackageReference Include="VeloxDev.WPF" Version="9.0.0" />` (or the project reference), and `dotnet build` compiles — the `VeloxDev.WorkflowSystem.AttachedBehaviors` namespace is now resolvable in XAML and code.
