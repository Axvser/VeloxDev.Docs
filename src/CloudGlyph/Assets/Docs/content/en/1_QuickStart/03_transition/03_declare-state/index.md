...
**Index arguments.** A path may carry index or array segments as well as property segments — `x.Items[i].Width`, `x.Map["player"].Color`, `x.Cells[1, 2]`. Their arguments are part of the path's **identity**, not of its value: `x => x.Items[idx].Width` parses to one path whatever `idx` happens to be, which is why a loop declaring five paths from five captured locals gets five entries. A non-frozen argument is re-evaluated by default, so the path **follows** a moving index (a captured local, or a property of the target such as `x.SelectedIndex`) — but the **end value was read once**, when the run started, so a path whose index moves mid-flight writes an end value computed against the slot it started on. Wrap the argument in `PathIndex.Frozen(...)` to pin it to the slot it resolves to at start; that is what you want whenever the end value must land where it was read from. A constant argument needs no marker and is frozen however you write it, and `Items[i]` and `Items[Frozen(i)]` are two different paths. (Source: `Src/Core/VeloxDev.Core/TransitionSystem/PathIndex.cs`; parsing and identity are pinned by `Src/Core/VeloxDev.Core.Test/TransitionSystem/TransitionPropertyIndexerTests.cs`.)
