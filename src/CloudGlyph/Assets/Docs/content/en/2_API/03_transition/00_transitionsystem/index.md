# Transition — Core Contracts: `VeloxDev.TransitionSystem`

This section documents the core, UI-agnostic contracts of the animation engine. They are declared in the `VeloxDev.TransitionSystem` namespace of the `VeloxDev.Core` assembly (source: `Src/Core/VeloxDev.Core/Interfaces/TransitionSystem/*.cs` and `Src/Core/VeloxDev.Core/TransitionSystem/*.cs`) and never reference any platform type. Platform adapters implement these contracts with concrete types documented in [03_adapter-provided](../03_adapter-provided/index.md).

## Roles

The engine separates five concerns, each expressed as interfaces:

- **Sampling** — `ISampler` interpolates one property per normalized time; `ISampleable` lets a custom type declare its own animatable members (struct assembly) without registering a sampler.
- **Property addressing** — `ITransitionProperty` names a (possibly nested) property path; `IFrameState` is the bag of recorded values / samplers / options for one snapshot.
- **Timing** — `IEaseCalculator`, `ITransitionEffectCore` / `ITransitionEffect<TPriorityCore>` describe how a single animation pass behaves.
- **Execution** — `ITransitionSchedulerCore` serializes animations per target; `ITransitionInterpreterCore` runs the sampling loop.
- **UI marshaling** — `IUIThreadInspectorCore` answers thread questions and marshals reads/writes to the UI thread.

The remaining members of this namespace — the `RotationDirection` enum and the `Eases` factory / concrete ease classes — are listed with the sampling contracts.

## Sub-pages

- [00_sampling-capture](00_sampling-capture/index.md) — `ISampler`, `ISampleable`, `ITransitionProperty`, `IFrameState` (sampling + property capture contracts).
- [01_effect-engine](01_effect-engine/index.md) — `ITransitionEffectCore` / `ITransitionEffect<TPriorityCore>`, the scheduler and interpreter interfaces, and `IUIThreadInspectorCore`.
- [02_eases](02_eases/index.md) — `RotationDirection`, `Eases`, and the 31 concrete ease classes.
