...
- Each adapter exposes non-generic `Transition : TransitionCore` and `Transition<T> : TransitionCore<T, State, TransitionEffect, Interpolator, UIThreadInspector, TransitionInterpreter, TPriorityCore>` — you normally call `Transition<T>.Create()`, `Transition.Exit(...)`, and the inherited instance `Execute(target, CanMutualTask)` (see [adapter-provided](../03_adapter-provided/index.md)). `AddNoMutual` / `RemoveNoMutual` / `RejectUnsampleablePaths` are `internal`. *Verified by:* WPF demo `MainWindow.xaml.cs`.
