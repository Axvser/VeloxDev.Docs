...
单一最小可运行程序：控制台 `net9.0`，开启 `<ImplicitUsings>enable</ImplicitUsings>`，引用 [01 安装 / 添加依赖](../01_install/index.md) 的 `VeloxDev.Core` + `VeloxDev.Core.Extension`（含源生成器）。本程序把前几页的 `CalcTree / SlotViewModel / CalcNode(+Helper)` 与四段执行逻辑拼成一份文件 —— 每个标识符都在本文件内定义。
