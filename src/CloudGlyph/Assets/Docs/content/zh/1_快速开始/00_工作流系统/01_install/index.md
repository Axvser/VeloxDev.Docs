...
`VeloxDev.Core.Extension` 提供 `VeloxDev.MVVM.Serialization.ComponentModelEx`（`Serialize` / `Deserialize`），本指南在 [06 序列化](../06_serialization/index.md) 用到。
...
依赖就绪后，在 [02 定义组件](../02_define-components/index.md) 声明 Tree / Slot / Node 与节点 Helper。
