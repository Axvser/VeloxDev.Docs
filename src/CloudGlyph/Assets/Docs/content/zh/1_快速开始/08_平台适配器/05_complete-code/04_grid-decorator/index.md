...
`dotnet new wpf-v-decorator -n GridDecorator -ns Demo.Views.Workflow` 的精确输出。基于 `Grid` 的装饰层，实现 Core 的 `IWorkflowGridDecorator` 契约：底层为网格层，最顶层为命中测试透明的浮动标尺层。每次布局/滚动时，表面行为把 `ScrollOffsetX/Y` 与 `ContentOffsetX/Y` 推入 DP 支撑的属性；`RulerBand` 供给空间虚拟化。
