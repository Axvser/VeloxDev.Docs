...
`VeloxDev.WPF.Templates` 包提供**七个项模板**，可搭出已接好行为的工作流视图。在[安装](../01_install/index.md)一节创建的 WPF 工程根目录运行。下面的类名不是随意取的：`WorkflowView.xaml` 会按名字引用 `NodeView`、`LinkView`、`GridDecorator`、`MinimapOverlay` 与 `TemplateSelector`，而 `NodeView` 内嵌 `SlotView`，所以要把它们放进同一个命名空间。
...
每个生成文件的完整内容都在[完整代码](../05_complete-code/index.md)子页面里逐字重现。
