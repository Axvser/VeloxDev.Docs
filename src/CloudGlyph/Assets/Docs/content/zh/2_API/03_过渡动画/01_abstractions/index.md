...
`VeloxDev.Core` 程序集中的 `VeloxDev.TransitionSystem.Abstractions` 命名空间（源码：`Src/Core/VeloxDev.Core/TransitionSystem/*.cs`）。这些具体 / 抽象基础类型实现了 [transitionsystem](../00_transitionsystem/index.md) 记录的契约。各平台适配器子类化它们，产出你实际构造的 `VeloxDev.TransitionSystem` 类型（见 [适配器提供](../03_适配器提供/index.md)）。
...
`IFrameState` 的具体默认实现；适配器的 `State` 派生自它（见 [适配器提供](../03_适配器提供/index.md)）。
...
- 这些是骨架类——每个抽象成员（线程身份、编组、读编组、存活）都由各适配器的 `UIThreadInspector` 填入（见 [适配器提供](../03_适配器提供/index.md)）。
