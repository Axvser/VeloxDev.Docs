...
- `TransitionCoreEx` —— 构建与运行 `StateSnapshotCore` 链的静态扩展（`Await`、`Then`、`AwaitThen`、`Interpolator`、`Execute`）→ [01_abstractions](../../01_abstractions/index.md)。
- 各适配器的 `TransitionEx`、`Transition`、`Transition<T>`、`Transition<T>.StateSnapshot` → [03_adapter-provided](../../03_adapter-provided/index.md)。
