...
| `EventType` | 当前生命周期状态（`CommandEventType`，见 [04_CommandEventType](../04_CommandEventType/index.md)）。 |
...
- 唯一的生产者是 `VeloxCommand`；使用者通过 `IVeloxCommand` 生命周期事件订阅（见 [02_IVeloxCommand](../02_IVeloxCommand/index.md)）并读取这些属性。
