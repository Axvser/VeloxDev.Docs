...
`Src/Core/VeloxDev.Core/AspectOriented/ProxyEx.cs`。静态工厂与钩子注册辅助类。该文件同时也声明了 `ProxyMembers` 枚举（见 [ProxyMembers 与 ProxyHandler](../01_proxy-members-handler/index.md)）。
...
**备注：** 通常应调用生成的 `Aop(this T)` 扩展而非直接使用 `CreateProxy`；见 [生成器产物](../../01_generated-api/index.md)。
...
相关页面：[ProxyInstance](../03_proxyinstance/index.md) 存储并分发所注册的三元组；[Aop 与 AopCache](../04_proxy-lifecycle/index.md) 说明 `Aop()` 的缓存机制。
