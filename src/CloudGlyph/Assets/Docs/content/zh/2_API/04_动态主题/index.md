...
| 支撑引擎 | `VeloxDev.TransitionSystem` / `.Abstractions` / `VeloxDev.Timing` | `InterpolatorCore`、`TransitionSchedulerCore`、`TimeSourceCore`（`ITimeSourceControl` 的默认实现）、`TransitionCore`、`ISampler`、`ISampleable`、`ITransitionProperty` / `TransitionProperty`、`ITransitionEffectCore`、`IEaseCalculator`、`Eases` |
...
- **支撑引擎** — 静态采样器注册表 `InterpolatorCore.NativeInterpolators`（`ConcurrentDictionary<Type, ISampler>`）、平台接缝 `InterpolatorCore.CreateScheduler` / `TransitionSchedulerCore` / 共享 `ITimeSourceControl`、采样契约 `ISampler`/`ISampleable`、属性契约 `TransitionProperty`，以及效果/缓动契约 `ITransitionEffectCore` / `IEaseCalculator` / `Eases`。
